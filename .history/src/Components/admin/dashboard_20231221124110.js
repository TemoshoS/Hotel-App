...
import { getBookings ,getCanceledBookings} from '../../services/roomServices';
...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchCanceledBookings = async () => {
      try {
        const canceledBookingsData = await getCanceledBookings();
        setCanceledBookings(canceledBookingsData);
      } catch (error) {
  ...
  const getStatusButtonClass = (checkInDate, checkOutDate, bookingStatus) => {
    const status = getStatus(checkInDate, checkOutDate, bookingStatus);
                  ...
                  {canceledBookings.map((booking) => (
                    <tr key={booking.id}>
                      ...
                      <td>
                        ...
                        >
                          {getStatus(booking.checkInDate, booking.checkOutDate,booking.status)}
                        </button>
                      </td>
                    </tr>
                  ))}
