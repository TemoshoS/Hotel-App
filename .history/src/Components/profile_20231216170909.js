...
function Profile() {
  ...
  const handleSave = async () => {
    if (selectedImage) {
      ...
      // If no image selected, update user information without photoURL
      await AuthService.updateUserInformation({
        displayName: updatedName,
        email: updatedEmail,
      });
              ...
              <tbody>
                {renderBookingHistory().map((booking) => (
                  <tr key={booking.id} className="booking-item">
                    ...
                    <td><button className={`statusBtn ${booking.status}`}>
                      {booking.status}
                    </button></td>
                    <td>
                      {selectedOption === 'present' ? (
                        <button onClick={() => handleCancelBooking(booking.id)} className='cancelBtn'>
                          free cancellation
                        </button>
                      ) : null}
                    </td>
